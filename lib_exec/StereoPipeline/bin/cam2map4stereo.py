...
def camrange( cube, options ):
    ...
    try:
        ...
        lonGroupName = 'UniversalGroundRange'
        lonDomain    = '360'
        lonDirection = 'PositiveEast'
        if options.map:
            # Read the pertinent values from the map file
            lonDomainIn    = subprocess.Popen([getkey_path, 'from= '+options.map, "grpname= Mapping", "keyword= LongitudeDomain"   ], stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].strip()
            lonDirectionIn = subprocess.Popen([getkey_path, 'from= '+options.map, "grpname= Mapping", "keyword= LongitudeDirection"], stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].strip()
            if lonDomainIn:
        ...
        # extract information
        info.resolution = subprocess.Popen([getkey_path, fromString, "grpname= PixelResolution",      "keyword= Lowest"          ], stdout=subprocess.PIPE).communicate()[0].strip()
        info.minlat     = subprocess.Popen([getkey_path, fromString, "grpname= UniversalGroundRange", "keyword= MinimumLatitude" ], stdout=subprocess.PIPE).communicate()[0].strip()
        info.maxlat     = subprocess.Popen([getkey_path, fromString, "grpname= UniversalGroundRange", "keyword= MaximumLatitude" ], stdout=subprocess.PIPE).communicate()[0].strip()
        info.minlon     = subprocess.Popen([getkey_path, fromString, lonGroupString,                  "keyword= MinimumLongitude"], stdout=subprocess.PIPE).communicate()[0].strip()
        info.maxlon     = subprocess.Popen([getkey_path, fromString, lonGroupString,                  "keyword= MaximumLongitude"], stdout=subprocess.PIPE).communicate()[0].strip()
...
def main():
    try:
        ...
        # Call
        image1 = camrange( args[0], options)
        image2 = camrange( args[1], options)
